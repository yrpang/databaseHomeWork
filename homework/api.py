...
parser_associationmentItem.add_argument('societyName', required=True,
...
parser_associationItem.add_argument('societyLoc', required=True,
                                   type=str, help="societyLoc not provide.")   # TO BE DELETE
...
class associationItem(Resource):
    def checkIfExist(self, societyNo):  # 查询是否存在
        ...
        cur.execute("SELECT * FROM Association WHERE societyNo='%s'" % societyNo)
        if(len(cur.fetchall()) < 1):
...
class association(Resource):
    ...
    def post(self):
        ...
        try:
            cur.execute("INSERT INTO Association(societyNo, societyName,societyYear,societyLoc) VALUES('%s', '%s', '%d', '%s');" % (
                args['society'], args['societyName'], args['societyYear'], args['societyLoc']))
            db.commit()
        except Error:
            return {'errCode': -1, 'status': '执行错误'}
        return {'errCode': 0, 'status': 'OK'}, 200
...
api.add_resource(associationItem, '/association/<string:societyNo>')
